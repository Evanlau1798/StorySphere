  ...
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-else>
      <div class="flex justify-between items-center mb-6">
        ...
        <button @click="submitForm" class="bg-green-500 text-white px-6 py-2 rounded-lg shadow hover:bg-green-600 transition-colors" :disabled="isSaving">
          {{ isSaving ? '儲存中...' : '儲存變更' }}
        </button>
      </div>
              ...
              <div v-for="volume in volumes" :key="volume.id" class="border rounded-lg overflow-hidden shadow-sm bg-gray-50 dark:bg-gray-700">
                <div class="flex justify-between items-center p-3 sm:p-4 bg-gray-100 dark:bg-gray-800">
                  ...
                  <div class="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
                    ...
                    <button @click="editVolume(volume)" class="btn-icon-label bg-yellow-500 text-white">
...
const volumes = ref<Volume[]>([]); // New state for volumes
const newVolumeTitle = ref(''); // New state for new volume input
const chapters = ref<Chapter[]>([]); // Keep for chapters without volume
const coverImageFile = ref<File | null>(null);
const coverPreview = ref<string | null>(null);

const isLoading = ref(true);
const isSaving = ref(false);
const volumesLoading = ref(true); // New loading state for volumes
...
const fetchNovelData = async () => {
  ...
  try {
    ...
    // Directly use the chapters_without_volume from the response
...
const addVolume = async () => {
  ...
  try {
    ...
    volumes.value.push({ ...response.data, chapters: [] }); // Add new volume with empty chapters array
...
const editVolume = async (volume: Volume) => {
  const newTitle = prompt('請輸入新的分卷標題：', volume.title);
  if (newTitle && newTitle.trim() !== volume.title) {
    isSaving.value = true;
    try {
      const response = await apiClient.patch(`/novels/${novelId.value}/volumes/${volume.id}/`, {
        title: newTitle.trim(),
      });
      const index = volumes.value.findIndex(v => v.id === volume.id);
      if (index !== -1) {
        volumes.value[index].title = response.data.title;
      }
      eventBus.emit(EventType.ShowAlert, {
        type: 'success',
        title: '成功',
...
const deleteVolume = async (volumeId: number) => {
  if (confirm('確定要刪除此分卷嗎？此操作將會把分卷下的章節移至未分卷章節。')) {
    ...
    try {
      ...
      // Re-fetch chapters to update the unassigned list
      fetchNovelData(); 
      eventBus.emit(EventType.ShowAlert, {
        type: 'success',
        title: '成功',
...
const deleteChapter = async (chapterId: number) => {
  if (confirm('確定要刪除此章節嗎？此操作無法復原。')) {
    ...
    try {
      ...
      // Remove from chapters without volume
